...
配置文件位置：config/config.toml（TOML 格式）
加载流程：
  1. 读取 config/config.toml 文件
  2. 解析为 AppConfig 对象（Pydantic 模型）
  3. 通过全局单例 config 提供给其他模块使用

使用方式：
  from app.config import config
  model = config.llm["default"].model   # 获取默认 LLM 模型名称

这个文件还定义了各种配置的数据模型（LLMSettings、BrowserSettings 等），
每个模型对应配置文件中的一个配置段。
"""
...
# 项目根目录：通过当前文件的位置推算（config.py 在 app/ 下，所以上一级就是项目根目录）
...
# ============================================================
# 配置数据模型（每个类对应配置文件中的一个配置段）
# ============================================================

class LLMSettings(BaseModel):
    """大语言模型配置

    对应 config.toml 中的 [llm] 段，包括：
    - model: 模型名称（如 gpt-4、qwen-max）
    - base_url: API 地址（支持第三方兼容 OpenAI 的服务）
    - api_key: 访问密钥
    - api_type: 接口类型（openai / azure / ollama）
    """
...
class SearchSettings(BaseModel):
    ...
    控制 Agent 使用哪个搜索引擎，以及失败时的回退策略。
    支持 Google / Baidu / Bing / DuckDuckGo 四个搜索引擎。
    """
...
class BrowserSettings(BaseModel):
    ...
    控制 browser-use 库的行为（Playwright 浏览器自动化）。
    - headless: 无头模式（不显示浏览器窗口，适合服务器环境）
    - disable_security: 禁用安全功能（允许访问任意网站）
    """
...
class Config:
    ...
    为什么用单例？
    配置只需要加载一次，整个应用的所有模块都读取同一份配置。
    单例模式确保不会出现多份配置副本不一致的问题。

    双重检查锁：
    使用 threading.Lock 确保多线程环境下也只创建一次实例。
    """
...
# 创建全局配置单例，其他模块通过 from app.config import config 使用
config = Config()
